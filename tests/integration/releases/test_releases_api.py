...
def test_release_production_gate_traceability_and_closure(client: TestClient) -> None:
    ...
    deployed = client.get(f"/api/v1/releases/{release_id}", headers=headers).json()
    ...
    still_deployed = client.get(
        f"/api/v1/releases/{release_id}", headers=headers
    ).json()
    ...
    closed_release = client.get(
        f"/api/v1/releases/{release_id}", headers=headers
    ).json()
    assert closed_release["status"] == "closed"
