...
class JiraService:
    ...
    def push_issue(self, data: JiraIssuePushCreate) -> JiraIssuePush:
        ...
        if existing is not None:
            return existing
        issue_key = data.simulated_jira_key or domain.simulate_jira_key()
        row = JiraIssuePush(
            id=uuid4(),
    ...
    def receive_status_webhook(self, data: JiraStatusWebhookIn) -> JiraStatusConflict:
    ...
    def create_comment_sync(self, data: JiraCommentSyncCreate) -> JiraCommentSync:
        ...
        row = JiraCommentSync(
            ...
            sync_status="failed" if data.force_fail else "synced",
            retry_count=1,
            failure_reason="Simulated Jira comment sync failure" if data.force_fail else None,
            last_attempt_at=now,
            created_by_actor_id=self.ctx.actor_id,
            updated_by_actor_id=self.ctx.actor_id,
        )
