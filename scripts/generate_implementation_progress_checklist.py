...
STATUS: dict[str, tuple[str, str]] = {
    ...
    # API
    "MOD-000-API-001": ("partial", "CRUD/query/transition endpoints exist; dedicated history endpoint not yet"),
    "MOD-000-API-002": ("partial", "Optimistic concurrency + idempotency + problem errors; pagination/filter/sort not full"),
    "MOD-000-API-003": ("partial", "OpenAPI via FastAPI models; explicit failure examples not fully curated"),
    # FE
    "MOD-000-FE-001": ("", "Deferred — web placeholder only"),
    "MOD-000-FE-002": ("", "Deferred — web placeholder only"),
    "MOD-000-FE-003": ("", "Deferred — web placeholder only"),
    "MOD-000-FE-004": ("", "Deferred — web placeholder only"),
    # WF
    "MOD-000-WF-001": ("partial", "Statuses/transitions/approvals/closure defined in docs + domain; waits/SLA not Temporal-backed"),
    "MOD-000-WF-002": ("n/a", "No long-running waits in MOD-000 stub; FastAPI owns mutations"),
    "MOD-000-WF-003": ("partial", "Correlation IDs on requests/audit; outbox/consumers deferred"),
    ...
    "MOD-010-MP-001": ("partial", "apps/ packages/ migrations/ tests/ infrastructure/ scripts/ skeleton present"),
    "MOD-010-MP-002": ("partial", "Python 3.12 / Node 22 chosen provisionally in PENDING_DECISIONS/ADR-0002"),
    "MOD-010-MP-003": ("partial", "uv working + lockfile; pnpm Corepack EPERM blocked"),
    "MOD-010-MP-004": ("partial", "docker-compose.yml has postgres+redis; not verified running"),
    "MOD-010-MP-005": ("partial", "ruff configured and passing for API/tests"),
    "MOD-010-MP-006": ("partial", "mypy configured and passing for masms_api"),
    "MOD-010-MP-007": ("partial", "pytest harness works; broader suite not started"),
    "MOD-010-MP-008": ("", "CI workflow not created"),
    "MOD-010-AC-003": ("partial", ".env.example has no real secrets; git repo not initialized yet"),
}
