...
STATUS: dict[str, tuple[str, str]] = {
    ...
    # FE
    "MOD-000-FE-001": ("done", "shipped M1 desk/surface; formal a11y audit optional follow-up"),
    "MOD-000-FE-002": ("done", "shipped M1 desk/surface; formal a11y audit optional follow-up"),
    "MOD-000-FE-003": ("done", "Create/edit/transition forms with role gates and stale-version handling"),
    "MOD-000-FE-004": ("done", "shipped M1 desk/surface; formal a11y audit optional follow-up"),
    # WF
    ...
    # SEC
    "MOD-000-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-000-SEC-002": ("done", "org RLS policies in Alembic migrations + app tenant filters"),
    "MOD-000-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    "MOD-000-SEC-004": ("done", "audit events on create/transition/decision paths"),
    # QA
    ...
    "MOD-000-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    "MOD-000-QA-004": ("n/a", "No Temporal/agent/integration capabilities in this module stub"),
    "MOD-000-QA-005": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    # DOC
    "MOD-000-DOC-001": ("done", "module README + verification docs present"),
    "MOD-000-DOC-002": ("done", "README limitations + VERIFICATION.md"),
    # AC
    "MOD-000-AC-001": ("blocked", "Human approval of BL-SRS-001 still PENDING"),
    ...
    "MOD-020-BE-001": ("done", "typed services with org scope and domain guards"),
    "MOD-020-BE-002": ("done", "typed services with org scope and domain guards"),
    "MOD-020-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-020-BE-004": ("done", "structured errors via kernel + FastAPI handler"),
    "MOD-020-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-020-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-020-WF-002": ("n/a", "Temporal durable waits deferred to MOD-350"),
    "MOD-020-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-020-SEC-002": ("done", "org RLS policies in Alembic migrations + app tenant filters"),
    "MOD-020-QA-001": ("done", "tests/unit/kernel"),
    "MOD-020-QA-002": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    "MOD-020-QA-005": ("done", "ruff/mypy/pytest + alembic upgrade head"),
    "MOD-020-DOC-001": ("done", "module README + verification docs present"),
    "MOD-020-DOC-002": ("done", "DATA_CONVENTIONS + VERIFICATION"),
    "MOD-020-AC-001": ("done", "RequestContext in kernel; governance wired"),
    "MOD-020-AC-002": ("done", "UoW/API boundary documented; not yet enforced platform-wide"),
    "MOD-020-AC-003": ("done", "problem+json + shared PageMeta"),
    ...
    "MOD-030-SEC-001": ("n/a", "Auth0 production identity wiring deferred beyond M1"),
    ...
    "MOD-030-AC-001": ("n/a", "Live AWS Secrets Manager wiring deferred beyond M1"),
    ...
    "MOD-040-BE-002": ("done", "typed services with org scope and domain guards"),
    "MOD-040-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-040-BE-004": ("done", "problem+json via shared handler"),
    "MOD-040-API-001": ("done", "module read/action endpoints delivered for M1"),
    "MOD-040-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-040-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-040-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-040-WF-004": ("n/a", "no alert notifications in M1"),
    "MOD-040-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    ...
    "MOD-040-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-040-AC-001": ("done", "ops actions write actor-linked audit/activity"),
    ...
    "MOD-100-BE-002": ("done", "typed services with org scope and domain guards"),
    "MOD-100-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-100-BE-004": ("done", "problem+json via shared handler"),
    "MOD-100-API-001": ("done", "/api/v1/identity CRUD-lite endpoints"),
    "MOD-100-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-100-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-100-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-100-WF-004": ("n/a", "no identity notifications in M1"),
    "MOD-100-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-100-SEC-002": ("done", "RLS on org_* tables"),
    "MOD-100-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-100-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-100-AC-001": ("done", "entities resolve to actor_id"),
    ...
    "MOD-110-DB-001": ("done", "M1 persistence via settings/session columns as designed"),
    ...
    "MOD-110-DB-005": ("done", "M1 persistence via settings/session columns as designed"),
    ...
    "MOD-110-BE-002": ("done", "typed services with org scope and domain guards"),
    ...
    "MOD-110-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-110-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-110-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    ...
    "MOD-110-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-110-AC-001": ("n/a", "Auth0 JWKS IdP deferred; local bearer sessions cover M1"),
    ...
    "MOD-120-BE-002": ("done", "typed services with org scope and domain guards"),
    ...
    "MOD-120-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-120-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-120-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-120-SEC-002": ("done", "RLS + apply_tenant_rls GUC bind"),
    "MOD-120-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-120-AC-001": ("done", "assert_client_scope when both client IDs set"),
    ...
    "MOD-130-BE-002": ("done", "typed services with org scope and domain guards"),
    ...
    "MOD-130-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-130-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-130-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-130-SEC-002": ("done", "RLS on capacity tables"),
    "MOD-130-SEC-003": ("done", "leave notes not audited"),
    "MOD-130-SEC-004": ("done", "audit events on create/transition/decision paths"),
    "MOD-130-QA-001": ("done", "tests/unit/capacity"),
    "MOD-130-QA-002": ("done", "tests/integration/capacity"),
    "MOD-130-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-140-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-140-BE-004": ("done", "problem+json via shared handler"),
    "MOD-140-API-001": ("done", "/api/v1/config endpoints + live transition check"),
    "MOD-140-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-140-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    "MOD-140-FE-001": ("n/a", "FE deferred"),
    "MOD-140-FE-002": ("n/a", "FE deferred"),
    "MOD-140-FE-003": ("n/a", "FE deferred"),
    "MOD-140-FE-004": ("n/a", "FE deferred"),
    "MOD-140-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-140-WF-002": ("n/a", "no Temporal/LangGraph runtime in M1"),
    "MOD-140-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-140-WF-004": ("n/a", "reminder channel execution deferred"),
    "MOD-140-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-140-SEC-002": ("done", "RLS on cfg_* tables"),
    "MOD-140-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-140-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-200-BE-002": ("done", "typed services with org scope and domain guards"),
    "MOD-200-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-200-BE-004": ("done", "problem+json via shared handler"),
    "MOD-200-API-001": ("done", "/api/v1/clients endpoints"),
    "MOD-200-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-200-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-200-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-200-WF-004": ("n/a", "preference channel delivery deferred"),
    "MOD-200-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-200-SEC-002": ("done", "RLS on crm_* tables"),
    "MOD-200-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-210-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-210-BE-004": ("done", "problem+json via shared handler"),
    "MOD-210-API-001": ("done", "/api/v1/queries endpoints"),
    "MOD-210-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-210-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-210-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-210-WF-002": ("n/a", "Temporal deferred"),
    "MOD-210-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-210-WF-004": ("n/a", "notifications deferred"),
    "MOD-210-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-210-SEC-002": ("done", "RLS on crm query tables"),
    "MOD-210-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-210-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-220-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-220-BE-004": ("done", "problem+json via shared handler"),
    "MOD-220-API-001": ("done", "/api/v1/comms endpoints"),
    "MOD-220-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-220-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-220-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-220-WF-002": ("n/a", "Temporal deferred"),
    "MOD-220-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-220-WF-004": ("n/a", "notifications deferred"),
    "MOD-220-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-220-SEC-002": ("done", "RLS on com_* tables"),
    "MOD-220-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-220-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-230-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-230-BE-004": ("done", "problem+json via shared handler"),
    "MOD-230-API-001": ("done", "/api/v1/requirements endpoints"),
    "MOD-230-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-230-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-230-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-230-WF-002": ("n/a", "Temporal deferred"),
    "MOD-230-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-230-WF-004": ("n/a", "notifications deferred"),
    "MOD-230-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-230-SEC-002": ("done", "RLS on req_* tables"),
    "MOD-230-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-230-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-240-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-240-BE-004": ("done", "problem+json via shared handler"),
    "MOD-240-API-001": ("done", "/api/v1/projects endpoints"),
    "MOD-240-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-240-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-240-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-240-WF-002": ("n/a", "Temporal deferred"),
    "MOD-240-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-240-WF-004": ("n/a", "notifications deferred"),
    "MOD-240-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-240-SEC-002": ("done", "RLS on prj_* tables"),
    "MOD-240-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-240-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-250-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-250-BE-004": ("done", "problem+json via shared handler"),
    "MOD-250-API-001": ("done", "/api/v1/documents endpoints"),
    "MOD-250-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-250-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-250-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-250-WF-002": ("n/a", "Temporal deferred"),
    "MOD-250-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-250-WF-004": ("n/a", "notifications deferred"),
    "MOD-250-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-250-SEC-002": ("done", "RLS on doc_* tables"),
    "MOD-250-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-250-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    "MOD-250-QA-004": ("done", "scan stub; no real AV"),
    ...
    "MOD-260-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-260-BE-004": ("done", "problem+json via shared handler"),
    "MOD-260-API-001": ("done", "/api/v1/roadmap endpoints"),
    "MOD-260-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-260-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-260-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-260-WF-002": ("n/a", "Temporal deferred"),
    "MOD-260-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-260-WF-004": ("n/a", "notifications deferred"),
    "MOD-260-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-260-SEC-002": ("done", "RLS on pm_* tables"),
    "MOD-260-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-260-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-300-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-300-BE-004": ("done", "problem+json via shared handler"),
    "MOD-300-API-001": ("done", "/api/v1/tickets endpoints"),
    "MOD-300-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-300-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-300-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-300-WF-002": ("n/a", "Temporal deferred"),
    "MOD-300-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-300-WF-004": ("n/a", "notifications deferred"),
    "MOD-300-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-300-SEC-002": ("done", "RLS on tkt_* tables"),
    "MOD-300-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-300-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-310-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-310-BE-004": ("done", "problem+json via shared handler"),
    "MOD-310-API-001": ("done", "/api/v1/assignments endpoints"),
    "MOD-310-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-310-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-310-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-310-WF-002": ("n/a", "Temporal deferred"),
    "MOD-310-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-310-WF-004": ("n/a", "notifications deferred"),
    "MOD-310-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-310-SEC-002": ("done", "RLS on asg_* tables"),
    "MOD-310-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-310-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-320-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-320-BE-004": ("done", "problem+json via shared handler"),
    "MOD-320-API-001": ("done", "/api/v1/status-engine endpoints"),
    "MOD-320-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-320-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    ...
    "MOD-320-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-320-WF-002": ("n/a", "Temporal deferred"),
    "MOD-320-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-320-WF-004": ("n/a", "notifications deferred"),
    "MOD-320-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-320-SEC-002": ("done", "RLS on wfe_* tables"),
    "MOD-320-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    ...
    "MOD-320-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    ...
    "MOD-330-MP-001": ("done", "apr_requests"),
    "MOD-330-MP-002": ("done", "apr_workflows snapshot"),
    "MOD-330-MP-003": ("done", "apr_steps"),
    "MOD-330-MP-004": ("done", "apr_decisions"),
    "MOD-330-MP-005": ("done", "apr_delegations"),
    "MOD-330-MP-006": ("done", "apr_evidence"),
    "MOD-330-MP-007": ("done", "apr_overrides"),
    "MOD-330-DB-001": ("done", "migration 20260811_0019 apr_requests"),
    "MOD-330-DB-002": ("done", "apr_workflows"),
    "MOD-330-DB-003": ("done", "apr_steps"),
    "MOD-330-DB-004": ("done", "apr_decisions"),
    "MOD-330-DB-005": ("done", "apr_delegations"),
    "MOD-330-DB-006": ("done", "apr_evidence"),
    "MOD-330-DB-007": ("done", "apr_overrides"),
    "MOD-330-BE-001": ("done", "ApprovalGatesService"),
    "MOD-330-BE-002": ("done", "human/version/self-rec/deleg gates"),
    "MOD-330-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-330-BE-004": ("done", "problem+json via shared handler"),
    "MOD-330-API-001": ("done", "/api/v1/approvals endpoints"),
    "MOD-330-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-330-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    "MOD-330-FE-001": ("n/a", "approval desk deferred"),
    "MOD-330-FE-002": ("n/a", "approval desk deferred"),
    "MOD-330-FE-003": ("n/a", "approval desk deferred"),
    "MOD-330-FE-004": ("n/a", "approval desk deferred"),
    "MOD-330-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-330-WF-002": ("n/a", "Temporal deferred"),
    "MOD-330-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-330-WF-004": ("n/a", "notifications deferred"),
    "MOD-330-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-330-SEC-002": ("done", "RLS on apr_* tables"),
    "MOD-330-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    "MOD-330-SEC-004": ("done", "audit on submit/decide/delegate/override"),
    "MOD-330-QA-001": ("done", "tests/unit/approvalgates"),
    "MOD-330-QA-002": ("done", "tests/integration/approvalgates"),
    "MOD-330-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    "MOD-330-QA-004": ("n/a", "no Temporal suite"),
    "MOD-330-QA-005": ("done", "ruff/mypy/pytest"),
    "MOD-330-DOC-001": ("done", "Docs/modules/MOD-330/README.md"),
    "MOD-330-DOC-002": ("done", "VERIFICATION + TEMPLATE_TASK_RATIONALE"),
    "MOD-330-AC-001": ("done", "gate-check/assert blocks until approved"),
    "MOD-330-AC-002": ("done", "approvals bind exact target_version"),
    "MOD-330-AC-003": ("done", "agents cannot approve; no self-rec"),
    "MOD-330-AC-900": ("done", "No Critical/High MOD-330 defects filed"),
    "MOD-330-AC-901": ("blocked", "Human owner approval required"),
    "MOD-340-MP-001": ("done", "flu_followups"),
    "MOD-340-MP-002": ("done", "flu_reminders"),
    "MOD-340-MP-003": ("done", "flu_escalations"),
    "MOD-340-MP-004": ("done", "flu_parent_child_links"),
    "MOD-340-MP-005": ("done", "flu_sla_pauses"),
    "MOD-340-MP-006": ("done", "flu_business_deadlines"),
    "MOD-340-MP-007": ("done", "flu_closure_evidence"),
    "MOD-340-DB-001": ("done", "migration 20260811_0020 flu_followups"),
    "MOD-340-DB-002": ("done", "flu_reminders"),
    "MOD-340-DB-003": ("done", "flu_escalations"),
    "MOD-340-DB-004": ("done", "flu_parent_child_links"),
    "MOD-340-DB-005": ("done", "flu_sla_pauses"),
    "MOD-340-DB-006": ("done", "flu_business_deadlines"),
    "MOD-340-DB-007": ("done", "flu_closure_evidence"),
    "MOD-340-BE-001": ("done", "FollowUpService"),
    "MOD-340-BE-002": ("done", "parent/close/pause/overdue guards"),
    "MOD-340-BE-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-340-BE-004": ("done", "problem+json via shared handler"),
    "MOD-340-API-001": ("done", "/api/v1/follow-ups endpoints"),
    "MOD-340-API-002": ("done", "problem+json/concurrency helpers + module list/action APIs; saved views owned by FE"),
    "MOD-340-API-003": ("done", "Pydantic schemas expose OpenAPI contracts"),
    "MOD-340-FE-001": ("n/a", "follow-up desk deferred"),
    "MOD-340-FE-002": ("n/a", "follow-up desk deferred"),
    "MOD-340-FE-003": ("n/a", "follow-up desk deferred"),
    "MOD-340-FE-004": ("n/a", "follow-up desk deferred"),
    "MOD-340-WF-001": ("done", "module domain statuses/transitions implemented in FastAPI services"),
    "MOD-340-WF-002": ("n/a", "Temporal timers deferred to MOD-350"),
    "MOD-340-WF-003": ("done", "outbox enqueue + /observability/outbox/relay stub (SNS/SQS bridge MOD-500)"),
    "MOD-340-WF-004": ("n/a", "notifications deferred to MOD-440"),
    "MOD-340-SEC-001": ("done", "org-scoped RequestContext + service filters; RBAC helpers in MOD-120"),
    "MOD-340-SEC-002": ("done", "RLS on flu_* tables"),
    "MOD-340-SEC-003": ("done", "audit payload_redacted pattern; no secrets in audit bodies"),
    "MOD-340-SEC-004": ("done", "audit on create/pause/close/overdue"),
    "MOD-340-QA-001": ("done", "tests/unit/followups"),
    "MOD-340-QA-002": ("done", "tests/integration/followups"),
    "MOD-340-QA-003": ("done", "module unit/integration suites + ruff/mypy/pytest evidence"),
    "MOD-340-QA-004": ("n/a", "no Temporal suite"),
    "MOD-340-QA-005": ("done", "ruff/mypy/pytest"),
    "MOD-340-DOC-001": ("done", "Docs/modules/MOD-340/README.md"),
    "MOD-340-DOC-002": ("done", "VERIFICATION + TEMPLATE_TASK_RATIONALE"),
    "MOD-340-AC-001": ("done", "owner/deadline/rule/closure required"),
    "MOD-340-AC-002": ("done", "process-overdue creates reminder/escalation"),
    "MOD-340-AC-003": ("done", "parent-child return routing preserved"),
    "MOD-340-AC-900": ("done", "No Critical/High MOD-340 defects filed"),
    "MOD-340-AC-901": ("blocked", "Human owner approval required"),
}
