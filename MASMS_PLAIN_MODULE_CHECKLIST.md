...
- Main task group (example: API)
  - [ ] First sub-task (`MOD-xxx-API-001`)
  - [ ] Second sub-task (`MOD-xxx-API-002`)
```
...
Plan IDs (like `MOD-000-API-001`) are shown in backticks for traceability.
...
- [x] **Main goals**
  ...
  - [x] Build and verify: requirement mapping (`MOD-000-MP-002`)
  - [x] Build and verify: architecture decision records (`MOD-000-MP-003`)
  - [x] Build and verify: change requests (`MOD-000-MP-004`)
  - [x] Build and verify: approval records (`MOD-000-MP-005`)

- [x] **Database**
  - [x] Design and migrate data for: baseline register (`MOD-000-DB-001`)
  - [x] Design and migrate data for: requirement mapping (`MOD-000-DB-002`)
  - [x] Design and migrate data for: architecture decision records (`MOD-000-DB-003`)
  - [x] Design and migrate data for: change requests (`MOD-000-DB-004`)
  - [x] Design and migrate data for: approval records (`MOD-000-DB-005`)

- [x] **Backend**
  - [x] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-000-BE-001`)
  - [x] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-000-BE-002`)
  - [-] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-000-BE-003`)
  - [x] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-000-BE-004`)

- [x] **API**
  - [x] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-000-API-001`)
...
- [x] **Workflow / agents / events**
  ...
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-000-WF-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: language versions (`MOD-010-MP-002`)
  - [x] Build and verify: package managers (`MOD-010-MP-003`)
  - [x] Build and verify: Docker Compose (`MOD-010-MP-004`)
  - [x] Build and verify: formatting and linting (`MOD-010-MP-005`)
  - [x] Build and verify: typing (`MOD-010-MP-006`)
  - [x] Build and verify: tests (`MOD-010-MP-007`)
  - [x] Build and verify: CI build (`MOD-010-MP-008`)

- [~] **Database**
  - [-] Design and migrate data for: monorepo structure (`MOD-010-DB-001`)
  - [-] Design and migrate data for: language versions (`MOD-010-DB-002`)
  - [-] Design and migrate data for: package managers (`MOD-010-DB-003`)
  - [-] Design and migrate data for: Docker Compose (`MOD-010-DB-004`)
  - [-] Design and migrate data for: formatting and linting (`MOD-010-DB-005`)
  - [-] Design and migrate data for: typing (`MOD-010-DB-006`)
  - [-] Design and migrate data for: tests (`MOD-010-DB-007`)
  - [-] Design and migrate data for: CI build (`MOD-010-DB-008`)

- [~] **Backend**
  - [-] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-010-BE-001`)
  - [-] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-010-BE-002`)
  - [-] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-010-BE-003`)
  - [-] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-010-BE-004`)

- [~] **API**
  - [-] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-010-API-001`)
  - [-] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-010-API-002`)
  - [-] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-010-API-003`)

- [~] **Frontend**
  - [-] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-010-FE-001`)
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-010-FE-002`)
  - [-] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-010-FE-003`)
  - [-] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-010-FE-004`)

- [~] **Workflow / agents / events**
  - [-] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-010-WF-001`)
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-010-WF-002`)
  - [-] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-010-WF-003`)
  - [-] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-010-WF-004`)

- [x] **Security / audit**
  - [-] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-010-SEC-001`)
  - [-] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-010-SEC-002`)
  - [x] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-010-SEC-003`)
  - [-] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-010-SEC-004`)

- [x] **Testing**
  - [-] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-010-QA-001`)
  - [-] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-010-QA-002`)
  - [-] Add role-permission negative tests and tenant/project isolation tests. (`MOD-010-QA-003`)
  - [-] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-010-QA-004`)
  - [x] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-010-QA-005`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: actor context (`MOD-020-MP-002`)
  - [x] Build and verify: tenant context (`MOD-020-MP-003`)
  - [x] Build and verify: domain errors (`MOD-020-MP-004`)
  - [x] Build and verify: unit of work (`MOD-020-MP-005`)
  - [x] Build and verify: outbox (`MOD-020-MP-006`)
  - [x] Build and verify: API problem details (`MOD-020-MP-007`)
  - [x] Build and verify: pagination (`MOD-020-MP-008`)
  - [x] Build and verify: optimistic concurrency (`MOD-020-MP-009`)

- [x] **Database**
  - [x] Design and migrate data for: typed identifiers (`MOD-020-DB-001`)
  - [x] Design and migrate data for: actor context (`MOD-020-DB-002`)
  - [x] Design and migrate data for: tenant context (`MOD-020-DB-003`)
  - [x] Design and migrate data for: domain errors (`MOD-020-DB-004`)
  - [x] Design and migrate data for: unit of work (`MOD-020-DB-005`)
  - [x] Design and migrate data for: outbox (`MOD-020-DB-006`)
  - [x] Design and migrate data for: API problem details (`MOD-020-DB-007`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: secret manager (`MOD-030-MP-002`)
  - [x] Build and verify: CI pipelines (`MOD-030-MP-003`)
  - [x] Build and verify: staging deployment (`MOD-030-MP-004`)
  - [x] Build and verify: production approval placeholder (`MOD-030-MP-005`)
  - [x] Build and verify: infrastructure as code (`MOD-030-MP-006`)

- [~] **Database**
  - [-] Design and migrate data for: environment matrix (`MOD-030-DB-001`)
  - [-] Design and migrate data for: secret manager (`MOD-030-DB-002`)
  - [-] Design and migrate data for: CI pipelines (`MOD-030-DB-003`)
  - [-] Design and migrate data for: staging deployment (`MOD-030-DB-004`)
  - [-] Design and migrate data for: production approval placeholder (`MOD-030-DB-005`)
  - [-] Design and migrate data for: infrastructure as code (`MOD-030-DB-006`)

- [~] **Backend**
  - [-] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-030-BE-001`)
  - [-] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-030-BE-002`)
  - [-] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-030-BE-003`)
  - [-] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-030-BE-004`)

- [~] **API**
  - [-] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-030-API-001`)
  - [-] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-030-API-002`)
  - [-] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-030-API-003`)

- [~] **Frontend**
  - [-] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-030-FE-001`)
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-030-FE-002`)
  - [-] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-030-FE-003`)
  - [-] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-030-FE-004`)

- [~] **Workflow / agents / events**
  - [-] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-030-WF-001`)
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-030-WF-002`)
...
- [x] **Testing**
  - [x] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-030-QA-001`)
  - [-] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-030-QA-002`)
  - [-] Add role-permission negative tests and tenant/project isolation tests. (`MOD-030-QA-003`)
  - [-] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-030-QA-004`)
  - [x] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-030-QA-005`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: activity events (`MOD-040-MP-002`)
  - [x] Build and verify: status history (`MOD-040-MP-003`)
  - [x] Build and verify: agent runs (`MOD-040-MP-004`)
  - [x] Build and verify: integration events (`MOD-040-MP-005`)
  - [x] Build and verify: OpenTelemetry (`MOD-040-MP-006`)
  - [x] Build and verify: health checks (`MOD-040-MP-007`)

- [x] **Database**
  - [x] Design and migrate data for: audit logs (`MOD-040-DB-001`)
  - [x] Design and migrate data for: activity events (`MOD-040-DB-002`)
  - [x] Design and migrate data for: status history (`MOD-040-DB-003`)
  - [x] Design and migrate data for: agent runs (`MOD-040-DB-004`)
  - [x] Design and migrate data for: integration events (`MOD-040-DB-005`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-040-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: actors (`MOD-100-MP-002`)
  - [x] Build and verify: human users (`MOD-100-MP-003`)
  - [x] Build and verify: agents (`MOD-100-MP-004`)
  - [x] Build and verify: roles (`MOD-100-MP-005`)
  - [x] Build and verify: departments (`MOD-100-MP-006`)
  - [x] Build and verify: teams (`MOD-100-MP-007`)
  - [x] Build and verify: team members (`MOD-100-MP-008`)
  - [x] Build and verify: reporting lines (`MOD-100-MP-009`)

- [x] **Database**
  - [x] Design and migrate data for: organizations (`MOD-100-DB-001`)
  - [x] Design and migrate data for: actors (`MOD-100-DB-002`)
  - [x] Design and migrate data for: human users (`MOD-100-DB-003`)
  - [x] Design and migrate data for: agents (`MOD-100-DB-004`)
  - [x] Design and migrate data for: roles (`MOD-100-DB-005`)
  - [x] Design and migrate data for: departments (`MOD-100-DB-006`)
  - [x] Design and migrate data for: teams (`MOD-100-DB-007`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-100-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: token validation (`MOD-110-MP-002`)
  - [x] Build and verify: sessions (`MOD-110-MP-003`)
  - [x] Build and verify: MFA (`MOD-110-MP-004`)
  - [x] Build and verify: step-up authentication (`MOD-110-MP-005`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-110-FE-002`)
  - [-] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-110-FE-003`)
  - [-] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-110-FE-004`)

- [~] **Workflow / agents / events**
  - [-] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-110-WF-001`)
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-110-WF-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: role permissions (`MOD-120-MP-002`)
  - [x] Build and verify: project members (`MOD-120-MP-003`)
  - [x] Build and verify: module access (`MOD-120-MP-004`)
  - [x] Build and verify: document access (`MOD-120-MP-005`)
  - [x] Build and verify: approval authorities (`MOD-120-MP-006`)
  - [x] Build and verify: RLS policies (`MOD-120-MP-007`)
  - [x] Build and verify: access reviews (`MOD-120-MP-008`)

- [x] **Database**
  - [x] Design and migrate data for: permissions (`MOD-120-DB-001`)
  - [x] Design and migrate data for: role permissions (`MOD-120-DB-002`)
  - [x] Design and migrate data for: project members (`MOD-120-DB-003`)
  - [x] Design and migrate data for: module access (`MOD-120-DB-004`)
  - [x] Design and migrate data for: document access (`MOD-120-DB-005`)
  - [x] Design and migrate data for: approval authorities (`MOD-120-DB-006`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-120-FE-002`)
  - [-] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-120-FE-003`)
  - [-] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-120-FE-004`)

- [~] **Workflow / agents / events**
  - [-] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-120-WF-001`)
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-120-WF-002`)
...
- [x] **Testing**
  - [x] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-120-QA-001`)
  - [x] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-120-QA-002`)
  - [x] Add role-permission negative tests and tenant/project isolation tests. (`MOD-120-QA-003`)
  - [-] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-120-QA-004`)
  - [x] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-120-QA-005`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: actor skills (`MOD-130-MP-002`)
  - [x] Build and verify: availability (`MOD-130-MP-003`)
  - [x] Build and verify: capacity allocations (`MOD-130-MP-004`)
  - [x] Build and verify: business calendars (`MOD-130-MP-005`)
  - [x] Build and verify: holidays (`MOD-130-MP-006`)
  - [x] Build and verify: leave periods (`MOD-130-MP-007`)
  - [x] Build and verify: on-call schedules (`MOD-130-MP-008`)

- [x] **Database**
  - [x] Design and migrate data for: skills (`MOD-130-DB-001`)
  - [x] Design and migrate data for: actor skills (`MOD-130-DB-002`)
  - [x] Design and migrate data for: availability (`MOD-130-DB-003`)
  - [x] Design and migrate data for: capacity allocations (`MOD-130-DB-004`)
  - [x] Design and migrate data for: business calendars (`MOD-130-DB-005`)
  - [x] Design and migrate data for: holidays (`MOD-130-DB-006`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-130-FE-002`)
  - [-] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-130-FE-003`)
  - [-] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-130-FE-004`)

- [~] **Workflow / agents / events**
  - [-] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-130-WF-001`)
  - [-] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-130-WF-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: status definitions (`MOD-140-MP-002`)
  - [x] Build and verify: transition rules (`MOD-140-MP-003`)
  - [x] Build and verify: follow-up rules (`MOD-140-MP-004`)
  - [x] Build and verify: reminder rules (`MOD-140-MP-005`)
  - [x] Build and verify: escalation rules (`MOD-140-MP-006`)
  - [x] Build and verify: approval workflows (`MOD-140-MP-007`)
  - [x] Build and verify: configuration versions (`MOD-140-MP-008`)

- [x] **Database**
  - [x] Design and migrate data for: workflow definitions (`MOD-140-DB-001`)
  - [x] Design and migrate data for: status definitions (`MOD-140-DB-002`)
  - [x] Design and migrate data for: transition rules (`MOD-140-DB-003`)
  - [x] Design and migrate data for: follow-up rules (`MOD-140-DB-004`)
  - [x] Design and migrate data for: reminder rules (`MOD-140-DB-005`)
  - [x] Design and migrate data for: escalation rules (`MOD-140-DB-006`)
...
- [~] **Frontend**
  ...
  - [-] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-140-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: contacts (`MOD-200-MP-002`)
  - [x] Build and verify: project contacts (`MOD-200-MP-003`)
  - [x] Build and verify: communication preferences (`MOD-200-MP-004`)
  - [x] Build and verify: duplicate suggestions (`MOD-200-MP-005`)
  - [x] Build and verify: merge history (`MOD-200-MP-006`)

- [x] **Database**
  - [x] Design and migrate data for: clients (`MOD-200-DB-001`)
  - [x] Design and migrate data for: contacts (`MOD-200-DB-002`)
  - [x] Design and migrate data for: project contacts (`MOD-200-DB-003`)
  - [x] Design and migrate data for: communication preferences (`MOD-200-DB-004`)
...
- [x] **Frontend**
  ...
  - [x] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-200-FE-002`)
...
- [x] **Testing**
  - [x] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-200-QA-001`)
  - [x] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-200-QA-002`)
  - [x] Add role-permission negative tests and tenant/project isolation tests. (`MOD-200-QA-003`)
  - [-] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-200-QA-004`)
  - [x] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-200-QA-005`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: opportunities (`MOD-210-MP-002`)
  - [x] Build and verify: qualification answers (`MOD-210-MP-003`)
  - [x] Build and verify: query sources (`MOD-210-MP-004`)
  - [x] Build and verify: query status history (`MOD-210-MP-005`)
  - [x] Build and verify: first response SLA (`MOD-210-MP-006`)

- [x] **Database**
  - [x] Design and migrate data for: queries (`MOD-210-DB-001`)
  - [x] Design and migrate data for: opportunities (`MOD-210-DB-002`)
  - [x] Design and migrate data for: qualification answers (`MOD-210-DB-003`)
  - [x] Design and migrate data for: query sources (`MOD-210-DB-004`)
...
- [x] **Frontend**
  ...
  - [x] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-210-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: messages (`MOD-220-MP-002`)
  - [x] Build and verify: message revisions (`MOD-220-MP-003`)
  - [x] Build and verify: recipients (`MOD-220-MP-004`)
  - [x] Build and verify: delivery receipts (`MOD-220-MP-005`)
  - [x] Build and verify: attachment links (`MOD-220-MP-006`)

- [x] **Database**
  - [x] Design and migrate data for: conversations (`MOD-220-DB-001`)
  - [x] Design and migrate data for: messages (`MOD-220-DB-002`)
  - [x] Design and migrate data for: message revisions (`MOD-220-DB-003`)
  - [x] Design and migrate data for: recipients (`MOD-220-DB-004`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: questionnaire versions (`MOD-230-MP-002`)
  - [x] Build and verify: answers (`MOD-230-MP-003`)
  - [x] Build and verify: requirement briefs (`MOD-230-MP-004`)
  - [x] Build and verify: clarification requests (`MOD-230-MP-005`)
  - [x] Build and verify: completeness scoring (`MOD-230-MP-006`)

- [x] **Database**
  - [x] Design and migrate data for: questionnaires (`MOD-230-DB-001`)
  - [x] Design and migrate data for: questionnaire versions (`MOD-230-DB-002`)
  - [x] Design and migrate data for: answers (`MOD-230-DB-003`)
  - [x] Design and migrate data for: requirement briefs (`MOD-230-DB-004`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: requirements (`MOD-240-MP-002`)
  - [x] Build and verify: requirement versions (`MOD-240-MP-003`)
  - [x] Build and verify: business rules (`MOD-240-MP-004`)
  - [x] Build and verify: acceptance criteria (`MOD-240-MP-005`)
  - [x] Build and verify: assumptions (`MOD-240-MP-006`)
  - [x] Build and verify: constraints (`MOD-240-MP-007`)
  - [x] Build and verify: SRS baselines (`MOD-240-MP-008`)

- [x] **Database**
  - [x] Design and migrate data for: projects (`MOD-240-DB-001`)
  - [x] Design and migrate data for: requirements (`MOD-240-DB-002`)
  - [x] Design and migrate data for: requirement versions (`MOD-240-DB-003`)
  - [x] Design and migrate data for: business rules (`MOD-240-DB-004`)
  - [x] Design and migrate data for: acceptance criteria (`MOD-240-DB-005`)
  - [x] Design and migrate data for: assumptions (`MOD-240-DB-006`)
...
- [x] **Frontend**
  ...
  - [x] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-240-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: document versions (`MOD-250-MP-002`)
  - [x] Build and verify: templates (`MOD-250-MP-003`)
  - [x] Build and verify: template versions (`MOD-250-MP-004`)
  - [x] Build and verify: attachments (`MOD-250-MP-005`)
  - [x] Build and verify: document permissions (`MOD-250-MP-006`)
  - [x] Build and verify: scan results (`MOD-250-MP-007`)

- [x] **Database**
  - [x] Design and migrate data for: documents (`MOD-250-DB-001`)
  - [x] Design and migrate data for: document versions (`MOD-250-DB-002`)
  - [x] Design and migrate data for: templates (`MOD-250-DB-003`)
  - [x] Design and migrate data for: template versions (`MOD-250-DB-004`)
  - [x] Design and migrate data for: attachments (`MOD-250-DB-005`)
...
- [x] **Frontend**
  ...
  - [x] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-250-FE-002`)
...
- [x] **Main goals**
  ...
  - [x] Build and verify: milestones (`MOD-260-MP-002`)
  - [x] Build and verify: deliverables (`MOD-260-MP-003`)
  - [x] Build and verify: phase dependencies (`MOD-260-MP-004`)
  - [x] Build and verify: project baselines (`MOD-260-MP-005`)
  - [x] Build and verify: forecasts (`MOD-260-MP-006`)

- [x] **Database**
  - [x] Design and migrate data for: phases (`MOD-260-DB-001`)
  - [x] Design and migrate data for: milestones (`MOD-260-DB-002`)
  - [x] Design and migrate data for: deliverables (`MOD-260-DB-003`)
  - [x] Design and migrate data for: phase dependencies (`MOD-260-DB-004`)
...
- [x] **Frontend**
  ...
  - [x] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-260-FE-002`)
...
- [ ] **Main goals**
  - [ ] Build and verify: tickets (`MOD-300-MP-001`)
  - [ ] Build and verify: subtasks (`MOD-300-MP-002`)
  - [ ] Build and verify: ticket dependencies (`MOD-300-MP-003`)
  - [ ] Build and verify: requirement links (`MOD-300-MP-004`)
  - [ ] Build and verify: ticket evidence (`MOD-300-MP-005`)
  - [ ] Build and verify: readiness checks (`MOD-300-MP-006`)
  - [ ] Build and verify: done checks (`MOD-300-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: tickets (`MOD-300-DB-001`)
  - [ ] Design and migrate data for: subtasks (`MOD-300-DB-002`)
  - [ ] Design and migrate data for: ticket dependencies (`MOD-300-DB-003`)
  - [ ] Design and migrate data for: requirement links (`MOD-300-DB-004`)
  - [ ] Design and migrate data for: ticket evidence (`MOD-300-DB-005`)
  - [ ] Design and migrate data for: readiness checks (`MOD-300-DB-006`)
  - [ ] Design and migrate data for: done checks (`MOD-300-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-300-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-300-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-300-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-300-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-300-API-001`)
...
- [ ] **Security / audit**
  ...
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-300-SEC-002`)
  ...
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-300-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-300-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-300-QA-002`)
  ...
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-300-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-300-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-300-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] No ticket becomes Ready without required information. (`MOD-300-AC-001`)
  - [ ] Tickets link to project, phase, owner or queue, and requirement. (`MOD-300-AC-002`)
  - [ ] Done tickets reopen only with authority and evidence. (`MOD-300-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-300-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: workflow resolver (`MOD-320-MP-001`)
  - [ ] Build and verify: transition evaluator (`MOD-320-MP-002`)
  - [ ] Build and verify: status history (`MOD-320-MP-003`)
  - [ ] Build and verify: hold records (`MOD-320-MP-004`)
  - [ ] Build and verify: reopen records (`MOD-320-MP-005`)
  - [ ] Build and verify: available next actions (`MOD-320-MP-006`)

- [ ] **Database**
  - [ ] Design and migrate data for: workflow resolver (`MOD-320-DB-001`)
  - [ ] Design and migrate data for: transition evaluator (`MOD-320-DB-002`)
  - [ ] Design and migrate data for: status history (`MOD-320-DB-003`)
  - [ ] Design and migrate data for: hold records (`MOD-320-DB-004`)
  - [ ] Design and migrate data for: reopen records (`MOD-320-DB-005`)
  - [ ] Design and migrate data for: available next actions (`MOD-320-DB-006`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-320-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-320-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-320-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-320-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-320-API-001`)
...
- [ ] **Frontend**
  ...
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-320-FE-002`)
...
- [ ] **Security / audit**
  ...
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-320-SEC-002`)
  ...
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-320-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-320-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-320-QA-002`)
  ...
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-320-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-320-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-320-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] No business status is hard-coded as a database enum. (`MOD-320-AC-001`)
  - [ ] Every transition creates history and audit. (`MOD-320-AC-002`)
  - [ ] Agents cannot skip required approval gates. (`MOD-320-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-320-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: approvals (`MOD-330-MP-001`)
  - [ ] Build and verify: approval workflows (`MOD-330-MP-002`)
  - [ ] Build and verify: approval steps (`MOD-330-MP-003`)
  - [ ] Build and verify: approval decisions (`MOD-330-MP-004`)
  - [ ] Build and verify: delegations (`MOD-330-MP-005`)
  - [ ] Build and verify: approval evidence (`MOD-330-MP-006`)
  - [ ] Build and verify: human overrides (`MOD-330-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: approvals (`MOD-330-DB-001`)
  - [ ] Design and migrate data for: approval workflows (`MOD-330-DB-002`)
  - [ ] Design and migrate data for: approval steps (`MOD-330-DB-003`)
  - [ ] Design and migrate data for: approval decisions (`MOD-330-DB-004`)
  - [ ] Design and migrate data for: delegations (`MOD-330-DB-005`)
  - [ ] Design and migrate data for: approval evidence (`MOD-330-DB-006`)
  - [ ] Design and migrate data for: human overrides (`MOD-330-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-330-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-330-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-330-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-330-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-330-API-001`)
...
- [ ] **Frontend**
  ...
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-330-FE-002`)
...
- [ ] **Security / audit**
  ...
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-330-SEC-002`)
  ...
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-330-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-330-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-330-QA-002`)
  ...
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-330-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-330-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-330-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Dependent actions remain blocked until approval. (`MOD-330-AC-001`)
  - [ ] Approvals bind to exact versions. (`MOD-330-AC-002`)
  - [ ] Agents cannot approve their own recommendations. (`MOD-330-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-330-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: follow-ups (`MOD-340-MP-001`)
  - [ ] Build and verify: reminders (`MOD-340-MP-002`)
  - [ ] Build and verify: escalations (`MOD-340-MP-003`)
  - [ ] Build and verify: parent-child links (`MOD-340-MP-004`)
  - [ ] Build and verify: SLA pauses (`MOD-340-MP-005`)
  - [ ] Build and verify: business-time deadlines (`MOD-340-MP-006`)
  - [ ] Build and verify: closure evidence (`MOD-340-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: follow-ups (`MOD-340-DB-001`)
  - [ ] Design and migrate data for: reminders (`MOD-340-DB-002`)
  - [ ] Design and migrate data for: escalations (`MOD-340-DB-003`)
  - [ ] Design and migrate data for: parent-child links (`MOD-340-DB-004`)
  - [ ] Design and migrate data for: SLA pauses (`MOD-340-DB-005`)
  - [ ] Design and migrate data for: business-time deadlines (`MOD-340-DB-006`)
  - [ ] Design and migrate data for: closure evidence (`MOD-340-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-340-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-340-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-340-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-340-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-340-API-001`)
...
- [ ] **Frontend**
  ...
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-340-FE-002`)
...
- [ ] **Security / audit**
  ...
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-340-SEC-002`)
  ...
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-340-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-340-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-340-QA-002`)
  ...
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-340-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-340-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-340-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Every request has owner, deadline, rule version, and closure condition. (`MOD-340-AC-001`)
  - [ ] Overdue items trigger configured reminders and escalation. (`MOD-340-AC-002`)
  - [ ] Parent-child chains preserve return routing. (`MOD-340-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-340-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: workflow instances (`MOD-350-MP-001`)
  - [ ] Build and verify: workflow signals (`MOD-350-MP-002`)
  - [ ] Build and verify: workflow versions (`MOD-350-MP-003`)
  - [ ] Build and verify: workflow failures (`MOD-350-MP-004`)
  - [ ] Build and verify: interventions (`MOD-350-MP-005`)
  - [ ] Build and verify: 12 approved workflows (`MOD-350-MP-006`)

- [ ] **Database**
  - [ ] Design and migrate data for: workflow instances (`MOD-350-DB-001`)
  - [ ] Design and migrate data for: workflow signals (`MOD-350-DB-002`)
  - [ ] Design and migrate data for: workflow versions (`MOD-350-DB-003`)
  - [ ] Design and migrate data for: workflow failures (`MOD-350-DB-004`)
  - [ ] Design and migrate data for: interventions (`MOD-350-DB-005`)
  - [ ] Design and migrate data for: 12 approved workflows (`MOD-350-DB-006`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-350-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-350-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-350-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-350-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-350-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-350-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-350-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-350-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-350-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-350-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-350-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-350-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-350-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-350-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-350-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-350-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-350-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-350-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-350-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-350-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-350-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-350-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-350-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-350-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-350-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-350-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Workflows survive worker restarts. (`MOD-350-AC-001`)
  - [ ] Timers, retries, and duplicate signals are idempotent. (`MOD-350-AC-002`)
  - [ ] Workflow history does not replace PostgreSQL business state. (`MOD-350-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-350-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: knowledge items (`MOD-370-MP-001`)
  - [ ] Build and verify: knowledge versions (`MOD-370-MP-002`)
  - [ ] Build and verify: chunks (`MOD-370-MP-003`)
  - [ ] Build and verify: embeddings (`MOD-370-MP-004`)
  - [ ] Build and verify: knowledge permissions (`MOD-370-MP-005`)
  - [ ] Build and verify: usage logs (`MOD-370-MP-006`)
  - [ ] Build and verify: knowledge conflicts (`MOD-370-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: knowledge items (`MOD-370-DB-001`)
  - [ ] Design and migrate data for: knowledge versions (`MOD-370-DB-002`)
  - [ ] Design and migrate data for: chunks (`MOD-370-DB-003`)
  - [ ] Design and migrate data for: embeddings (`MOD-370-DB-004`)
  - [ ] Design and migrate data for: knowledge permissions (`MOD-370-DB-005`)
  - [ ] Design and migrate data for: usage logs (`MOD-370-DB-006`)
  - [ ] Design and migrate data for: knowledge conflicts (`MOD-370-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-370-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-370-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-370-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-370-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-370-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-370-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-370-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-370-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-370-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-370-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-370-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-370-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-370-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-370-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-370-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-370-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-370-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-370-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-370-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-370-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-370-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-370-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-370-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-370-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-370-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-370-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Agents cite the source and version used. (`MOD-370-AC-001`)
  - [ ] Project-approved knowledge outranks generic examples. (`MOD-370-AC-002`)
  - [ ] Unauthorized, expired, rejected, or superseded content is excluded. (`MOD-370-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-370-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: test cases (`MOD-400-MP-001`)
  - [ ] Build and verify: test steps (`MOD-400-MP-002`)
  - [ ] Build and verify: test suites (`MOD-400-MP-003`)
  - [ ] Build and verify: test plans (`MOD-400-MP-004`)
  - [ ] Build and verify: test runs (`MOD-400-MP-005`)
  - [ ] Build and verify: test evidence (`MOD-400-MP-006`)
  - [ ] Build and verify: coverage links (`MOD-400-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: test cases (`MOD-400-DB-001`)
  - [ ] Design and migrate data for: test steps (`MOD-400-DB-002`)
  - [ ] Design and migrate data for: test suites (`MOD-400-DB-003`)
  - [ ] Design and migrate data for: test plans (`MOD-400-DB-004`)
  - [ ] Design and migrate data for: test runs (`MOD-400-DB-005`)
  - [ ] Design and migrate data for: test evidence (`MOD-400-DB-006`)
  - [ ] Design and migrate data for: coverage links (`MOD-400-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-400-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-400-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-400-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-400-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-400-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-400-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-400-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-400-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-400-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-400-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-400-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-400-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-400-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-400-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-400-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-400-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-400-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-400-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-400-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-400-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-400-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-400-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-400-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-400-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-400-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-400-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Every Must-Have requirement has approved test coverage. (`MOD-400-AC-001`)
  - [ ] Critical permissions have negative tests. (`MOD-400-AC-002`)
  - [ ] Test evidence is tied to environment and build. (`MOD-400-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-400-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: bugs (`MOD-410-MP-001`)
  - [ ] Build and verify: bug links (`MOD-410-MP-002`)
  - [ ] Build and verify: bug assignments (`MOD-410-MP-003`)
  - [ ] Build and verify: fix submissions (`MOD-410-MP-004`)
  - [ ] Build and verify: retests (`MOD-410-MP-005`)
  - [ ] Build and verify: known issue approvals (`MOD-410-MP-006`)
  - [ ] Build and verify: severity SLA (`MOD-410-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: bugs (`MOD-410-DB-001`)
  - [ ] Design and migrate data for: bug links (`MOD-410-DB-002`)
  - [ ] Design and migrate data for: bug assignments (`MOD-410-DB-003`)
  - [ ] Design and migrate data for: fix submissions (`MOD-410-DB-004`)
  - [ ] Design and migrate data for: retests (`MOD-410-DB-005`)
  - [ ] Design and migrate data for: known issue approvals (`MOD-410-DB-006`)
  - [ ] Design and migrate data for: severity SLA (`MOD-410-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-410-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-410-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-410-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-410-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-410-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-410-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-410-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-410-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-410-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-410-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-410-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-410-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-410-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-410-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-410-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-410-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-410-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-410-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-410-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-410-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-410-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-410-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-410-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-410-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-410-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-410-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] QA can reject and reopen work with evidence. (`MOD-410-AC-001`)
  - [ ] Blocking defects prevent release. (`MOD-410-AC-002`)
  - [ ] Bug history links requirement, ticket, test, fix, retest, and release. (`MOD-410-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-410-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: notifications (`MOD-440-MP-001`)
  - [ ] Build and verify: preferences (`MOD-440-MP-002`)
  - [ ] Build and verify: templates (`MOD-440-MP-003`)
  - [ ] Build and verify: deliveries (`MOD-440-MP-004`)
  - [ ] Build and verify: retries (`MOD-440-MP-005`)
  - [ ] Build and verify: dead letters (`MOD-440-MP-006`)
  - [ ] Build and verify: digests (`MOD-440-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: notifications (`MOD-440-DB-001`)
  - [ ] Design and migrate data for: preferences (`MOD-440-DB-002`)
  - [ ] Design and migrate data for: templates (`MOD-440-DB-003`)
  - [ ] Design and migrate data for: deliveries (`MOD-440-DB-004`)
  - [ ] Design and migrate data for: retries (`MOD-440-DB-005`)
  - [ ] Design and migrate data for: dead letters (`MOD-440-DB-006`)
  - [ ] Design and migrate data for: digests (`MOD-440-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-440-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-440-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-440-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-440-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-440-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-440-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-440-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-440-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-440-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-440-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-440-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-440-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-440-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-440-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-440-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-440-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-440-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-440-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-440-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-440-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-440-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-440-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-440-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-440-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-440-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-440-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Notifications are timely, idempotent, auditable, and permission-safe. (`MOD-440-AC-001`)
  - [ ] Users can configure preferences without disabling mandatory critical alerts. (`MOD-440-AC-002`)
  - [ ] Delivery failures are visible and recoverable. (`MOD-440-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-440-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: integration connections (`MOD-500-MP-001`)
  - [ ] Build and verify: webhook events (`MOD-500-MP-002`)
  - [ ] Build and verify: sync cursors (`MOD-500-MP-003`)
  - [ ] Build and verify: external mappings (`MOD-500-MP-004`)
  - [ ] Build and verify: outbox events (`MOD-500-MP-005`)
  - [ ] Build and verify: inbox events (`MOD-500-MP-006`)
  - [ ] Build and verify: connection health (`MOD-500-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: integration connections (`MOD-500-DB-001`)
  - [ ] Design and migrate data for: webhook events (`MOD-500-DB-002`)
  - [ ] Design and migrate data for: sync cursors (`MOD-500-DB-003`)
  - [ ] Design and migrate data for: external mappings (`MOD-500-DB-004`)
  - [ ] Design and migrate data for: outbox events (`MOD-500-DB-005`)
  - [ ] Design and migrate data for: inbox events (`MOD-500-DB-006`)
  - [ ] Design and migrate data for: connection health (`MOD-500-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-500-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-500-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-500-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-500-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-500-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-500-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-500-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-500-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-500-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-500-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-500-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-500-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-500-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-500-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-500-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-500-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-500-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-500-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-500-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-500-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-500-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-500-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-500-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-500-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-500-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-500-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Integration failure cannot corrupt internal data. (`MOD-500-AC-001`)
  - [ ] External mappings and events are tenant-scoped and audited. (`MOD-500-AC-002`)
  - [ ] Credentials never appear in logs, prompts, tickets, or business tables. (`MOD-500-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-500-AC-900`)
  - [ ] The responsible human owner reviews and approves the completion evidence. (`MOD-500-AC-901`)

### Module 34: [ ] MOD-510 — Gmail Client Communication Integration

- [ ] **Main goals**
  - [ ] Build and verify: Gmail connection (`MOD-510-MP-001`)
  - [ ] Build and verify: history cursor (`MOD-510-MP-002`)
  - [ ] Build and verify: thread mappings (`MOD-510-MP-003`)
  - [ ] Build and verify: message mappings (`MOD-510-MP-004`)
  - [ ] Build and verify: attachment import (`MOD-510-MP-005`)
  - [ ] Build and verify: draft review (`MOD-510-MP-006`)
  - [ ] Build and verify: approved send (`MOD-510-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: Gmail connection (`MOD-510-DB-001`)
  - [ ] Design and migrate data for: history cursor (`MOD-510-DB-002`)
  - [ ] Design and migrate data for: thread mappings (`MOD-510-DB-003`)
  - [ ] Design and migrate data for: message mappings (`MOD-510-DB-004`)
  - [ ] Design and migrate data for: attachment import (`MOD-510-DB-005`)
  - [ ] Design and migrate data for: draft review (`MOD-510-DB-006`)
  - [ ] Design and migrate data for: approved send (`MOD-510-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-510-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-510-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-510-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-510-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-510-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-510-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-510-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-510-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-510-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-510-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-510-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-510-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-510-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-510-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-510-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-510-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-510-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-510-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-510-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-510-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-510-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-510-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-510-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-510-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-510-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-510-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] Valid emails create or update exactly one query and thread. (`MOD-510-AC-001`)
  - [ ] Approved outgoing email is sent and linked correctly. (`MOD-510-AC-002`)
  - [ ] Duplicate notifications do not duplicate records. (`MOD-510-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-510-AC-900`)
...
- [ ] **Main goals**
  - [ ] Build and verify: threat model (`MOD-600-MP-001`)
  - [ ] Build and verify: PII inventory (`MOD-600-MP-002`)
  - [ ] Build and verify: retention policies (`MOD-600-MP-003`)
  - [ ] Build and verify: legal holds (`MOD-600-MP-004`)
  - [ ] Build and verify: deletion jobs (`MOD-600-MP-005`)
  - [ ] Build and verify: backup records (`MOD-600-MP-006`)
  - [ ] Build and verify: restore tests (`MOD-600-MP-007`)
  - [ ] Build and verify: security incidents (`MOD-600-MP-008`)

- [ ] **Database**
  - [ ] Design and migrate data for: threat model (`MOD-600-DB-001`)
  - [ ] Design and migrate data for: PII inventory (`MOD-600-DB-002`)
  - [ ] Design and migrate data for: retention policies (`MOD-600-DB-003`)
  - [ ] Design and migrate data for: legal holds (`MOD-600-DB-004`)
  - [ ] Design and migrate data for: deletion jobs (`MOD-600-DB-005`)
  - [ ] Design and migrate data for: backup records (`MOD-600-DB-006`)
  - [ ] Design and migrate data for: restore tests (`MOD-600-DB-007`)
  - [ ] Design and migrate data for: security incidents (`MOD-600-DB-008`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-600-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-600-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-600-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-600-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-600-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-600-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-600-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-600-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-600-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-600-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-600-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-600-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-600-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-600-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-600-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-600-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-600-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-600-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-600-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-600-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-600-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-600-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-600-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-600-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-600-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-600-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] No Critical security or isolation defect remains. (`MOD-600-AC-001`)
  - [ ] RPO and RTO targets are validated. (`MOD-600-AC-002`)
  - [ ] Client and company data are excluded from model training by default. (`MOD-600-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-600-AC-900`)
  - [ ] The responsible human owner reviews and approves the completion evidence. (`MOD-600-AC-901`)

### Module 37: [ ] MOD-610 — Performance, Reliability, Idempotency, Resilience, and Disaster Recovery

- [ ] **Main goals**
  - [ ] Build and verify: performance tests (`MOD-610-MP-001`)
  - [ ] Build and verify: resilience tests (`MOD-610-MP-002`)
  - [ ] Build and verify: index review (`MOD-610-MP-003`)
  - [ ] Build and verify: SLO dashboards (`MOD-610-MP-004`)
  - [ ] Build and verify: workflow replay (`MOD-610-MP-005`)
  - [ ] Build and verify: integration failure tests (`MOD-610-MP-006`)
  - [ ] Build and verify: DR runbook (`MOD-610-MP-007`)

- [ ] **Database**
  - [ ] Design and migrate data for: performance tests (`MOD-610-DB-001`)
  - [ ] Design and migrate data for: resilience tests (`MOD-610-DB-002`)
  - [ ] Design and migrate data for: index review (`MOD-610-DB-003`)
  - [ ] Design and migrate data for: SLO dashboards (`MOD-610-DB-004`)
  - [ ] Design and migrate data for: workflow replay (`MOD-610-DB-005`)
  - [ ] Design and migrate data for: integration failure tests (`MOD-610-DB-006`)
  - [ ] Design and migrate data for: DR runbook (`MOD-610-DB-007`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-610-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-610-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-610-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-610-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-610-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-610-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-610-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-610-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-610-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-610-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-610-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-610-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-610-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-610-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-610-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-610-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-610-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-610-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-610-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-610-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-610-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-610-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-610-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-610-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-610-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-610-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] 95% of normal APIs are under two seconds. (`MOD-610-AC-001`)
  - [ ] Dashboard is under three seconds at pilot load. (`MOD-610-AC-002`)
  - [ ] Durable workflows resume after failure and remain idempotent. (`MOD-610-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-610-AC-900`)
  - [ ] The responsible human owner reviews and approves the completion evidence. (`MOD-610-AC-901`)

### Module 38: [ ] MOD-620 — Synthetic Sample Projects, Agent Evaluation, End-to-End Acceptance, and UAT

- [ ] **Main goals**
  - [ ] Build and verify: seed scripts (`MOD-620-MP-001`)
  - [ ] Build and verify: expected decisions (`MOD-620-MP-002`)
  - [ ] Build and verify: agent evaluations (`MOD-620-MP-003`)
  - [ ] Build and verify: E2E tests (`MOD-620-MP-004`)
  - [ ] Build and verify: role-based UAT (`MOD-620-MP-005`)
  - [ ] Build and verify: acceptance evidence (`MOD-620-MP-006`)

- [ ] **Database**
  - [ ] Design and migrate data for: seed scripts (`MOD-620-DB-001`)
  - [ ] Design and migrate data for: expected decisions (`MOD-620-DB-002`)
  - [ ] Design and migrate data for: agent evaluations (`MOD-620-DB-003`)
  - [ ] Design and migrate data for: E2E tests (`MOD-620-DB-004`)
  - [ ] Design and migrate data for: role-based UAT (`MOD-620-DB-005`)
  - [ ] Design and migrate data for: acceptance evidence (`MOD-620-DB-006`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-620-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-620-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-620-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-620-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-620-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-620-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-620-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-620-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-620-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-620-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-620-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-620-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-620-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-620-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-620-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-620-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-620-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-620-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-620-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-620-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-620-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-620-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-620-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-620-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-620-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-620-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] All three sample projects pass defined workflows. (`MOD-620-AC-001`)
  - [ ] Agent quality metrics meet targets. (`MOD-620-AC-002`)
  - [ ] No unauthorized agent approval or isolation failure occurs. (`MOD-620-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-620-AC-900`)
  - [ ] The responsible human owner reviews and approves the completion evidence. (`MOD-620-AC-901`)

### Module 39: [ ] MOD-630 — Controlled Pilot, Production Release, Operations, and Final MVP Sign-Off

- [ ] **Main goals**
  - [ ] Build and verify: pilot plan (`MOD-630-MP-001`)
  - [ ] Build and verify: pilot users (`MOD-630-MP-002`)
  - [ ] Build and verify: training (`MOD-630-MP-003`)
  - [ ] Build and verify: support readiness (`MOD-630-MP-004`)
  - [ ] Build and verify: known limitations (`MOD-630-MP-005`)
  - [ ] Build and verify: production deployment (`MOD-630-MP-006`)
  - [ ] Build and verify: rollback (`MOD-630-MP-007`)
  - [ ] Build and verify: final sign-offs (`MOD-630-MP-008`)

- [ ] **Database**
  - [ ] Design and migrate data for: pilot plan (`MOD-630-DB-001`)
  - [ ] Design and migrate data for: pilot users (`MOD-630-DB-002`)
  - [ ] Design and migrate data for: training (`MOD-630-DB-003`)
  - [ ] Design and migrate data for: support readiness (`MOD-630-DB-004`)
  - [ ] Design and migrate data for: known limitations (`MOD-630-DB-005`)
  - [ ] Design and migrate data for: production deployment (`MOD-630-DB-006`)
  - [ ] Design and migrate data for: rollback (`MOD-630-DB-007`)
  - [ ] Design and migrate data for: final sign-offs (`MOD-630-DB-008`)

- [ ] **Backend**
  - [ ] Implement typed domain models, commands, queries, repositories, and application services for the approved scope. (`MOD-630-BE-001`)
  - [ ] Enforce authorization, approval, status-transition, concurrency, and idempotency rules before mutation. (`MOD-630-BE-002`)
  - [ ] Publish domain events through the transactionally safe outbox when asynchronous processing is required. (`MOD-630-BE-003`)
  - [ ] Return structured errors for validation, forbidden, not found, conflict, invalid transition, and approval required. (`MOD-630-BE-004`)

- [ ] **API**
  - [ ] Create versioned CRUD, query, transition, action, and history endpoints required by the module. (`MOD-630-API-001`)
  - [ ] Add pagination, filtering, sorting, bounded search, optimistic concurrency, idempotency, and standard problem-details errors. (`MOD-630-API-002`)
  - [ ] Document request, response, validation, authorization, conflict, approval-required, invalid-transition, and not-found examples in OpenAPI. (`MOD-630-API-003`)

- [ ] **Frontend**
  - [ ] Create the module list or dashboard view with role-aware columns, filters, sorting, pagination, saved views, and empty/loading/error/forbidden states. (`MOD-630-FE-001`)
  - [ ] Create detail view tabs for summary, ownership, status, related records, documents, messages, follow-ups, approvals, audit, and activity where applicable. (`MOD-630-FE-002`)
  - [ ] Create create/edit/review forms with field validation, permission-aware actions, stale-version handling, confirmation, and accessible error messages. (`MOD-630-FE-003`)
  - [ ] Verify responsive layout, keyboard navigation, focus order, contrast, timezone rendering, and screen-reader labels. (`MOD-630-FE-004`)

- [ ] **Workflow / agents / events**
  - [ ] Define triggers, owners, inputs, outputs, statuses, transitions, waits, reminders, escalations, approvals, evidence, and closure rules. (`MOD-630-WF-001`)
  - [ ] Route long-running waits and timers through Temporal; route bounded reasoning through LangGraph; keep state changes in FastAPI services. (`MOD-630-WF-002`)
  - [ ] Define domain events, outbox publication, idempotent consumers, correlation IDs, retries, dead-letter behavior, and replay rules. (`MOD-630-WF-003`)
  - [ ] Define notification recipients, channels, content classification, quiet hours, priority overrides, delivery audit, and failure handling. (`MOD-630-WF-004`)

- [ ] **Security / audit**
  - [ ] Enforce organization, client, project, role, module, action, classification, environment, and effective-date authorization. (`MOD-630-SEC-001`)
  - [ ] Add tenant-isolation and project-isolation controls in application services and RLS where applicable. (`MOD-630-SEC-002`)
  - [ ] Minimize and redact PII, secrets, tokens, credentials, and restricted data in logs, prompts, notifications, events, exports, and errors. (`MOD-630-SEC-003`)
  - [ ] Create audit events for create, read-sensitive, update, delete, assignment, transition, approval, rejection, override, export, integration, and agent actions. (`MOD-630-SEC-004`)

- [ ] **Testing**
  - [ ] Add unit tests for domain rules, validation, conflicts, and invalid state. (`MOD-630-QA-001`)
  - [ ] Add integration and API-contract tests for transactions, persistence, errors, concurrency, and idempotency. (`MOD-630-QA-002`)
  - [ ] Add role-permission negative tests and tenant/project isolation tests. (`MOD-630-QA-003`)
  - [ ] Add workflow, agent, event, integration, file, security, or performance tests where the module uses those capabilities. (`MOD-630-QA-004`)
  - [ ] Run formatter, lint, type check, tests, migrations, frontend build, and relevant security or performance checks. (`MOD-630-QA-005`)

- [ ] **Docs**
  - [ ] Update module README, data dictionary, API documentation, permissions, status rules, approvals, events, audit catalog, operational notes, and user guidance. (`MOD-630-DOC-001`)
  - [ ] Record migration, rollback, known limitations, verification commands, and evidence references. (`MOD-630-DOC-002`)

- [ ] **Acceptance / Done gate**
  - [ ] All Critical and High acceptance tests pass. (`MOD-630-AC-001`)
  - [ ] Pilot users approve controlled production use. (`MOD-630-AC-002`)
  - [ ] Cross-functional production readiness sign-off is complete. (`MOD-630-AC-003`)
  - [ ] All Critical and High defects for this module are resolved. (`MOD-630-AC-900`)
  - [ ] The responsible human owner reviews and approves the completion evidence. (`MOD-630-AC-901`)
